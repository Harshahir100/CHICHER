...
import {
  ClipboardList, PhoneCall, Package, Truck, Home, XCircle,
  CheckCircle2, ChevronDown, ChevronUp, ShoppingBag, RefreshCw, Trash2, MapPin, Phone,
  AlertCircle,
} from 'lucide-react';
import { fetchOrders, updateOrderStatus, deleteOrder } from '@/lib/ordersApi';
import {
  ORDER_STAGES, STAGE_META, TERMINAL_STATUSES, stageIndex, formatStatusDate,
    ...
    <div>
      <div className="border-b border-ink-100 bg-ink-100/50">
        <div className="container-app py-10">
          <p className="text-sm font-medium uppercase tracking-widest text-brand-600">Your purchases</p>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h1 className="mt-1 font-display text-4xl font-bold text-ink-900">My Orders</h1>
      ...
      <div className="container-app py-8">
        ...
        {loading ? (
          <div className="space-y-4">
            {[1, 2].map((n) => (
              <div key={n} className="h-40 animate-pulse rounded-2xl bg-ink-100" />
            ))}
          </div>
        ) : orders.length === 0 ? (
          <div className="flex flex-col items-center gap-4 rounded-2xl border border-dashed border-ink-200 py-20 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-ink-500">Your placed orders will appear here with live status tracking.</p>
            </div>
...
function OrderCard({ order, expanded, onToggle, onAdvance, onCancel, onRemove, advancing }) {
    ...
    <div className="overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-soft">
      ...
      <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <div className={`grid h-12 w-12 shrink-0 place-items-center rounded-full ${
            isCancelled ? 'bg-error-500/10 text-error-600'
            : isDelivered ? 'bg-success-500/10 text-success-600'
            : 'bg-brand-50 text-brand-600'
          }`}>
            {isCancelled ? <XCircle className="h-6 w-6" /> : isDelivered ? <CheckCircle2 className="h-6 w-6" /> : <Package className="h-6 w-6" />}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <p className="font-display text-lg font-semibold text-ink-900">Order {order.id}</p>
              <StatusPill status={status} />
            </div>
            <p className="mt-0.5 text-sm text-ink-500">
              Placed on {formatStatusDate(order.created_at)} · {itemCount} item{itemCount !== 1 && 's'} · ₹{order.total.toLocaleString('en-IN')} (COD)
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="border-t border-ink-100 px-5 py-5">
        ...
        {!isCancelled && order.status_history?.length > 0 && (
          <p className="mt-3 text-xs text-ink-500">
            Last update: {meta.label} — {formatStatusDate(order.status_history[order.status_history.length - 1].at)}
          </p>
        )}
      </div>
      ...
      {expanded && (
        <div className="border-t border-ink-100 bg-ink-50/50 px-5 py-5 animate-fadeIn">
          {/* Items */}
          <h4 className="mb-3 text-sm font-semibold text-ink-900">Items in this order</h4>
          <div className="space-y-3">
            {order.items.map((item, i) => (
              <div key={i} className="flex gap-3 rounded-xl border border-ink-100 bg-white p-3">
                <Link to={`/product/${item.productId}`}>
                  <img src={item.image} alt={item.name} className="h-16 w-14 rounded-lg object-cover" />
                </Link>
                <div className="flex flex-1 flex-col">
                  <Link to={`/product/${item.productId}`} className="line-clamp-1 text-sm font-semibold text-ink-900 hover:text-brand-600">
                    {item.name}
                  </Link>
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            <div className="rounded-xl border border-ink-100 bg-white p-4">
              <h4 className="mb-2 text-sm font-semibold text-ink-900">Payment Summary</h4>
              <div className="space-y-1.5 text-sm">
                ...
                <div className="flex justify-between text-ink-600">
                  <span>Shipping</span>
                  <span>{order.shipping === 0 ? <span className="text-success-600">Free</span> : `₹${order.shipping}`}</span>
                </div>
                <div className="flex justify-between border-t border-ink-100 pt-1.5 font-bold">
                  <span>Total (COD)</span><span className="text-brand-700">₹{order.total.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
            <div className="rounded-xl border border-ink-100 bg-white p-4">
              <h4 className="mb-2 text-sm font-semibold text-ink-900">Delivery Address</h4>
              <p className="text-sm font-medium text-ink-900">{order.customer.fullName}</p>
              ...
              <p className="mt-1.5 flex items-center gap-1.5 text-sm text-ink-600">
                <Phone className="h-3.5 w-3.5 shrink-0 text-ink-400" /> {order.customer.phone}
              </p>
            </div>
          </div>
          ...
          {order.status_history?.length > 0 && (
            <div className="mt-5 rounded-xl border border-ink-100 bg-white p-4">
              <h4 className="mb-3 text-sm font-semibold text-ink-900">Status History</h4>
                ...
                {order.status_history.map((h, i) => {
                  ...
                  return (
                    <li key={i} className="flex items-center gap-3 text-sm">
                      ...
                      <span className="font-medium text-ink-900">{m.label || h.status}</span>
                      <span className="text-ink-400">·</span>
                      <span className="text-ink-500">{formatStatusDate(h.at)}</span>
                    </li>
                  );
                })}
              </ol>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {!isTerminal && (
              <>
                <button onClick={onAdvance} disabled={advancing} className="btn-primary text-sm">
                  {advancing ? 'Updating...' : 'Advance Status'}
                </button>
                <button onClick={onCancel} disabled={advancing} className="btn-outline text-sm">
                  Cancel Order
                </button>
              </>
            )}
            {isTerminal && (
              <button onClick={onRemove} className="btn-ghost text-sm text-error-600 hover:bg-error-500/10">
                <Trash2 className="h-4 w-4" /> Remove from list
              </button>
            )}
            <Link to="/" className="btn-ghost text-sm">Buy again</Link>
          </div>
        </div>
      )}
    </div>
        ...
        return (
          <div key={stage} className="flex flex-1 items-center gap-1 sm:gap-2">
            <div className="flex flex-col items-center gap-1.5">
              <div className={`grid h-9 w-9 shrink-0 place-items-center rounded-full border-2 transition-all ${
              ...
              <span className={`whitespace-nowrap text-[10px] font-medium sm:text-xs ${done ? 'text-ink-900' : 'text-ink-400'}`}>
                {meta.label}
              </span>
            </div>
            {i < ORDER_STAGES.length - 1 && (
              <div className={`h-0.5 flex-1 rounded-full transition-colors ${i < idx ? 'bg-brand-600' : 'bg-ink-200'}`} />
            )}
          </div>
        );
      })}

...
import {
  Heart, ShoppingCart, Minus, Plus, Star, Truck, Banknote,
  RefreshCw, ShieldCheck, ChevronRight, Check,
} from 'lucide-react';
...
export default function ProductDetail({ onCheckout }) {
  ...
  const [size, setSize] = useState(product?.sizes.length > 1 ? product.sizes[1] || product.sizes[0] : product?.sizes[0]);
    ...
    <div>
      ...
      <div className="container-app py-4">
        <nav className="flex items-center gap-1.5 text-sm text-ink-500">
          ...
          <Link to={`/category/${product.category}`} className="hover:text-brand-600 capitalize">
      ...
      <div className="container-app grid gap-10 pb-10 lg:grid-cols-2">
        ...
        <div className="lg:sticky lg:top-24 lg:self-start">
          <div className="relative overflow-hidden rounded-3xl bg-ink-100">
            ...
            <div className="absolute left-4 top-4 flex flex-col gap-1.5">
              {discount > 0 && <span className="badge bg-brand-600 text-white">-{discount}%</span>}
              {product.isNew && <span className="badge bg-accent-500 text-white">New</span>}
            </div>
          </div>
        ...
        {/* Info */}
        <div>
          <p className="text-sm font-medium uppercase tracking-wide text-brand-600">{product.subcategory}</p>
          <h1 className="mt-1 font-display text-3xl font-bold text-ink-900 sm:text-4xl">{product.name}</h1>
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-sm text-ink-500">({product.reviewsCount} reviews)</span>
          </div>
          ...
          <div className="mt-5 flex items-baseline gap-3">
            ...
            {product.originalPrice > product.price && (
              <span className="text-lg text-ink-400 line-through">₹{product.originalPrice.toLocaleString('en-IN')}</span>
            )}
            {discount > 0 && (
              <span className="badge bg-success-500/10 text-success-600">Save {discount}%</span>
            )}
          </div>
          <p className="mt-1 text-xs text-ink-500">Inclusive of all taxes · SKU: {product.sku}</p>
          ...
          <div className="mt-5 flex items-center gap-2 rounded-xl border border-success-500/20 bg-success-500/5 px-4 py-3">
            ...
            <p className="text-sm font-medium text-success-700">Available for Cash on Delivery</p>
          </div>

          {/* Stock */}
          <p className={`mt-4 text-sm font-medium ${product.inStock ? 'text-success-600' : 'text-error-500'}`}>
          ...
          <div className="mt-6">
            <p className="mb-2 text-sm font-semibold text-ink-900">
              Color: <span className="font-normal text-ink-600">{color?.name}</span>
            </p>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className={`grid h-12 w-12 place-items-center rounded-full border transition-all ${
                wished ? 'border-brand-600 bg-brand-600 text-white' : 'border-ink-200 bg-white text-ink-700 hover:border-brand-400'
              }`}

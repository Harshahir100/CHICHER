        ...
        <div className="container-app grid gap-8 py-12 md:grid-cols-2 md:items-center">
          <div>
            ...
            <p className="mt-2 text-sm text-ink-300">
              Subscribe for early access to new collections, styling tips, and member-only offers.
            </p>
          </div>
      ...
      <div className="container-app grid gap-10 py-14 sm:grid-cols-2 lg:grid-cols-4">
        <div>
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="font-display text-2xl font-bold text-white">Aurelia</span>
          </Link>
          <p className="mt-4 text-sm leading-relaxed text-ink-300">
            Handcrafted women's fashion and jewellery, blending ethnic heritage with modern design.
            Cash on Delivery available across India.
          </p>
          <div className="mt-5 flex gap-3">
            <a href="#" aria-label="Instagram" className="grid h-9 w-9 place-items-center rounded-full bg-ink-800 text-ink-200 transition-colors hover:bg-brand-600 hover:text-white">
              <Instagram className="h-4 w-4" />
            </a>
            <a href="#" aria-label="Facebook" className="grid h-9 w-9 place-items-center rounded-full bg-ink-800 text-ink-200 transition-colors hover:bg-brand-600 hover:text-white">
        ...
        <div>
          <h4 className="font-display text-lg font-semibold text-white">Shop</h4>
          <ul className="mt-4 space-y-2.5 text-sm">
            {shopLinks.map((l) => (
              <li key={l.to}>
                <Link to={l.to} className="text-ink-300 transition-colors hover:text-brand-400">
                  {l.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          <h4 className="font-display text-lg font-semibold text-white">Customer Care</h4>
          <ul className="mt-4 space-y-2.5 text-sm">
            {policyLinks.map((l) => (
              <li key={l.to}>
                <Link to={l.to} className="text-ink-300 transition-colors hover:text-brand-400">
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-ink-300">
            ...
            <li className="flex items-center gap-3">
              ...
              <a href="tel:+918001234567" className="hover:text-brand-400">+91 80012 34567</a>
            </li>
            <li className="flex items-center gap-3">
              <Mail className="h-4 w-4 shrink-0 text-brand-400" />
              <a href="mailto:care@aurelia.example" className="hover:text-brand-400">care@aurelia.example</a>
            </li>
          </ul>
          <div className="mt-5 rounded-xl border border-ink-800 bg-ink-900 p-3 text-xs text-ink-300">
            ...
            <p className="mt-1">Pay in cash when your order arrives at your doorstep.</p>
          </div>
        </div>
      </div>

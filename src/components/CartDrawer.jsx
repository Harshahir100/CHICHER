    ...
    <div className={`fixed inset-0 z-50 ${isCartOpen ? '' : 'pointer-events-none'}`} aria-hidden={!isCartOpen}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-ink-200 bg-white px-5 py-4">
          ...
          <button onClick={() => setIsCartOpen(false)} aria-label="Close cart" className="grid h-9 w-9 place-items-center rounded-full hover:bg-ink-100">
        ...
        {cart.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-ink-500">Discover pieces you'll love.</p>
            </div>
          ...
          <>
            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              {cart.map((item) => (
                <div key={item.key} className="flex gap-3 rounded-2xl border border-ink-100 bg-white p-3">
            ...
            <div className="border-t border-ink-200 bg-white p-5">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-ink-600">
                  <span>Subtotal</span>
                  <span className="font-medium text-ink-900">₹{subtotal.toLocaleString('en-IN')}</span>
                </div>
                <div className="flex justify-between text-ink-600">
                  <span>Shipping</span>
              ...
              <button onClick={() => setIsCartOpen(false)} className="btn-ghost mt-2 w-full">
                Continue Shopping
              </button>
            </div>
          </>
